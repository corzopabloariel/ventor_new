<ol class="breadcrumb bg-transparent p-0 border-0">
    ...
    @isset($data["elements"]["subpart"])
        <li class="breadcrumb-item"><a href="{{ route((auth()->guard('web')->check() ? 'order_part_subpart' : 'products_part_subpart'), ['part' => $data["elements"]["part"]["name_slug"], 'subpart' => $data["elements"]["subpart"]["name_slug"]]) }}">{{ $data["elements"]["subpart"]["name"] }}</a></li>
    @endisset
</ol>