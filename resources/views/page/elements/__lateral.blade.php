    ...
    <div class="@if(isset($data["part"]) && $data["part"]->name_slug == $part['slug'])show @endif collapse collapse--{{ $part['slug'] }}" id="collapse--{{ $part['slug'] }}">
        <div class="subparts">
            @foreach($part["subparts"] AS $subpart)
                @php
                $class = "";
                if (isset($data["subpart"]) && $data["subpart"]->name_slug == $subpart['slug'])
                    $class = "class=active";
                @endphp
                <a {{ $class }} href="{{ route((auth()->guard('web')->check() ? 'order_subpart' : 'subpart'), ['part' => $part['slug'], 'subpart' => $subpart['slug']]) }}" style="--hover-color: {{ $part['color']['color'] }}">{{ $subpart["name"] }}</a>
            @endforeach
        </div>
    </div>
@endforeach